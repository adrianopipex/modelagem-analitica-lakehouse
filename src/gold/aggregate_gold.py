...
Este script lê as tabelas da camada Silver, constrói as dimensões e a
tabela fato (modelo estrela) e grava o resultado nas tabelas da
camada Gold, prontas para consumo analítico/BI.
"""
...
def build_dim_cliente(spark: SparkSession):
    ...
    df.write.format("delta").mode("overwrite").saveAsTable("gold.dim_cliente")


def build_dim_produto(spark: SparkSession):
    df = spark.table("silver.produtos").select(
        "id_produto", "nome_produto", "categoria", "preco_unitario"
    )
    df.write.format("delta").mode("overwrite").saveAsTable("gold.dim_produto")


def build_dim_tempo(spark: SparkSession):
    df = (
        spark.table("silver.vendas")
        .select("data_venda")
        .distinct()
    ...
    df.write.format("delta").mode("overwrite").saveAsTable("gold.dim_tempo")


def build_fato_vendas(spark: SparkSession):
    vendas = spark.table("silver.vendas")
    dim_cliente = spark.table("gold.dim_cliente").select("id_cliente")
    dim_produto = spark.table("gold.dim_produto").select("id_produto")
    dim_tempo = spark.table("gold.dim_tempo").select(F.col("data").alias("data_venda"))
    ...
    fato.write.format("delta").mode("overwrite").saveAsTable("gold.fato_vendas")
